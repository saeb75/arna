...
- **İçerikte kullanıcıya özel hiçbir veri YOKTUR** (isim yok). Lint bunu `JSON.stringify(content)` içinde adı arayarak zorlar. Bu, içeriğin ileride kullanıcılar arasında **paylaşılabilmesinin** ön koşulu (önbellek anahtarı: focus × seviye × tema kümesi × `nativeLanguage` × `formatVersion`).
...
- **`practice.mustUse` = öğrencinin BİREBİR söyleyeceği kısa kalıplar** ("I think", "have you ever"), gramer tarifi DEĞİL. Bu alan metin eşleşmesiyle ölçüldüğü için tarif yazılınca ölçüm ölüyor, tek kelime yazılınca ("do") her turda tetiklenip sahneyi kesiyordu. Dersin tarifi `tutorNotes.target` ve `successCriteria`'da durur. Eşleşme kelime sınırına saygı duyar; `goalMet` sahneyi `PRACTICE_MIN_TURNS_BEFORE_GOAL` (4) turundan önce bitiremez; tur tavanının tabanı 8'dir.
...
- **Plan peşin, içerik tembel:** onboarding'de tek LLM çağrısıyla tüm seviyenin ders planı iskeleti (~45–60 satır: başlık/gramer/kelime teması/kişisel tema) üretilir; ders İÇERİĞİ derse ilk tıklamada üretilip kaydedilir. Tüm dersler açık — kullanıcı istediğine atlar, sıra zorunlu değil.
- **Otomatik seviye geçişi YOK:** kullanıcı ayarlardan seviye değiştirir → plan yeniden üretilir; ilerleme ve memory asla silinmez.
...
`user_profiles` · `programs` · `program_lessons` (plan satırları) · `lessons` (üretilmiş içerik JSONB + validation) · `sessions` (`state`: hafıza bloğu + oturum script'i) · `transcript_turns` · `memories` (pgvector 1536) · `session_summaries` · `llm_calls`
